...
MainComponent::MainComponent()
{
	...
	fftSizeSetting.addItem(std::to_string(1 << 5), 5);
	fftSizeSetting.addItem(std::to_string(1 << 7), 7);
	fftSizeSetting.addItem(std::to_string(1 << 10), 10);
	fftSizeSetting.onChange = [this] { fftSizeSettingCallback(); };
	fftSizeSetting.setSelectedId(5);
	fftSizeSettingCallback();

	addAndMakeVisible(&fftSizeSetting);
}
...
void MainComponent::getNextAudioBlock (const AudioSourceChannelInfo& bufferToFill)
{
	...
	for (int i = 0; i < b->getNumSamples(); i++) {
		for (int c = 0; c < b->getNumChannels(); c++) {
			buffer[c][i] = 0.5f * audioBuffer[counter];
			counter++;
			counter %= fftSize;
		}
	}
...
void MainComponent::paint (Graphics& g)
{
	...
	for (int i = 0; i < fftSize; i++) {
		//saw[i] = 1.0f - 2.0f * std::fmod( 0.5f + freq * i / (float)fftSize, 1.0f); //saw
		...
		if (true) {
			saw[i] = 0.0f;
			for (int j = 1; j < 0.0675f * fftSize; j++) {
				saw[i] += (1.0f / ((float)j * 2.0f)) * sin(MathConstants<float>::twoPi * ( freq * (float)j * (float)i / (float)fftSize ) );
			}
		}
...
void MainComponent::resized()
{
	...
	onlyHarmonics.setBounds(header.removeFromRight(header.getHeight()*2.0f));
	...
	fftSizeSetting.setBounds(comboboxspace.removeFromBottom(0.5f * rightheader.getHeight()));
}
